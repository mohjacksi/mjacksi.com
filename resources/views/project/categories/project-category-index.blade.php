...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{clean( trans('mjacksi-backend.project') , array('Attr.EnableID' => true))}} {{clean( trans('mjacksi-backend.categories') , array('Attr.EnableID' => true))}}</h6>
        </div>
        <div class="card-body">
                        ...
                        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">{{clean( trans('mjacksi-backend.name') , array('Attr.EnableID' => true))}}</th>
                                </tr>
                            </thead>
                            <tfoot>
                                <tr>
                                ...
                                @if($categories)
                                    @foreach($categories as $category)
                                        <tr>
                                            ...
                                            <td data-label="name">{{$category->name}}<p class="mb-0 mt-2"><a href="{{ route('project-category.edit', $category->id)  . '?language=' . request()->input('language')}}">{{clean( trans('mjacksi-backend.edit') , array('Attr.EnableID' => true))}}</a></p></td>
                                        </tr>
                                     @endforeach
                                @endif
                         ...
                         <form action="{{route('project-category.store')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    ...
                                    <div class="form-group">
                                        <strong>{{clean( trans('mjacksi-backend.category') , array('Attr.EnableID' => true))}}</strong>
