...
<div class="container-fluid">
    ...
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">{{clean( trans('mjacksi-backend.section_7_blog') , array('Attr.EnableID' => true))}}</h1>

    <!-- DataTales Example -->
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{clean( trans('mjacksi-backend.section_7_blog') , array('Attr.EnableID' => true))}}</h6>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                ...
                <div class="row">
                    <div class="col-lg-6">
                        ...
                        <a href="{{route('blog-setting.edit') . '?language=' . request()->input('language')}}" class="btn btn-primary btn-back">{{clean( trans('mjacksi-backend.back_blogpage') , array('Attr.EnableID' => true))}}</a>
                        @endif
                        <a href="{{route('post.create') . '?language=' . request()->input('language')}}" class="btn btn-primary btn-back">{{clean( trans('mjacksi-backend.create_article') , array('Attr.EnableID' => true))}}</a>
                    </div>
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">{{clean( trans('mjacksi-backend.id') , array('Attr.EnableID' => true))}}</th>
                        ...
                        @if($posts)
                            @foreach($posts as $post)
                                <tr>
                                    ...
                                    <td data-label="Photo"><img height="50" src="{{$post->photo ? '/public/images/media/' . $post->photo->file : '/public/img/200x200.png'}}" alt=""><p class="mb-0 mt-2"><a href="{{ route('post.edit', $post->id) . '?language=' . request()->input('language')}}">{{clean( trans('mjacksi-backend.edit') , array('Attr.EnableID' => true))}}</a></p></td>
